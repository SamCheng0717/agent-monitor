...
# ── 质量评分 (DeepSeek) ────────────────────────────────────────────────────
SCORE_PROMPT = """\
分析以下客服对话，检测 AI 回复是否违反系统提示词规则，并评分。

【必检违规项（每项违反均严重扣分）】
1. 重复索要微信：对话历史中顾客已提供过微信号或手机号，AI 仍再次要求留微信
2. 直接报具体价格：对于超声刀/热玛吉/丽珠兰/黄金微针/玻尿酸/肉毒素，AI 给出具体数字价格而非知识库中的区间范围，或自行编造价格数字
3. 第一人称：AI 回复出现「我/我们/我帮/我来」等第一人称
4. 违禁词：AI 回复出现「案例/知识库/数据库/系统/保证/绝对/百分之百/AI/机器人/智能客服/助手」
5. 工作时间无故索微信：08:30–23:00 时段，顾客仅询问项目信息，AI 却拒绝直接回答、只引导留微信

【一般扣分项】
- 顾客重复追问同一问题 2 次以上，AI 仍未正面回答
- 顾客出现负面情绪词（没用/烦/算了/什么破/不想理）
- 回复冗长，超过 4 句话

输出 JSON（只输出 JSON）：
{{"score": 0.35, "problems": ["直接报具体价格", "重复索要微信"], "customer_turn": "触发该问题的顾客原话", "bad_turn": "AI违规回复原文", "suggestion": "应该怎么回"}}
